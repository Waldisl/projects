...
bool            isActualPort(const std::string &port)
{
	size_t portLength;
...
bool			isActualIpv4(const std::string &ipAddress)
{
	int		cnt;
...
bool			isActualAddress(const std::string &address)
{
	size_t	passIdx;
	size_t	portIdx;
	...
	if (!isActualPort(address.substr(portIdx + 1, passIdx - portIdx - 1)))
		return (false);
	if (!isActualIpv4(address.substr(0, portIdx)))
		return (false);
