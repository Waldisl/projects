...
namespace ft
{
	...
	class map_iterator
	{
		protected:
			...
			node_pointer _ptr;
			bool _key_comp(node_pointer ptr1, node_pointer ptr2) {return ptr1->_data->first < ptr2->_data->first;}
		...
		public:
			...
			map_iterator() : _ptr(), _M_node(_ptr) {}
			node_pointer _M_node;
			map_iterator(const node_pointer& ptr) : _ptr(ptr), _M_node(_ptr) {}

			// преобразование iterator в const_iterator
			template < typename T >
			map_iterator(const map_iterator< T, node_pointer, typename enable_if< is_same< T, typename Container::pointer >::value, Container >::type >& ptr)
				: _ptr(ptr.base()) {}
	...
	bool operator==(const map_iterator< IteratorL, node_pointer, Container >& lhs, const map_iterator< IteratorR, node_pointer, Container >& rhs)
	{return lhs.base() == rhs.base();}

	template < typename Iterator, typename node_pointer, typename Container >
	bool operator==(const map_iterator< Iterator, node_pointer, Container >& lhs, const map_iterator< Iterator, node_pointer, Container >& rhs)
	{return lhs.base() == rhs.base();}
